...
const changeCurrentPage = (newCurrentPage, prevState) => {
  return { ...prevState, currentPage: newCurrentPage };
};

const submitFilterValue = ({ minPrice, maxPrice, data }, prevState) => {
  const newFilteredData = data.works.filter(
    (book) => book.price[0] >= minPrice && book.price[0] <= maxPrice
  );
  return { ...prevState, filteredData: newFilteredData, currentPage: 1 };
};
